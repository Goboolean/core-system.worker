...
func TestRecreation(t *testing.T) {
	for i := 0; i < 10; i++ {
		fmt.Println("TestRecreation: try:", i)
		err := RecreateBucket(rawInfluxClient, opts.Org, opts.TradeBucketName)
		if err != nil {
			t.Error(err)
			t.FailNow()
		}

		health, err := rawInfluxClient.Health(context.Background())
		if err != nil {
			t.Error(err)
			t.FailNow()
		}
...
func TestPastStock(t *testing.T) {
	...
	t.Run("데이터가 저장된 만큼, 데이터를 가져와야 한다.", func(t *testing.T) {
		...
		fetchJob, err := fetcher.NewPastStock(cursor, &job.UserParams{
			job.ProductID: "thisIsWrongProductID",
			job.StartDate: fmt.Sprint(start.Unix()),
